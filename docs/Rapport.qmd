...
Dette dokument giver overblik over resultatet for Folketingsvalget, og kan opdateres dynamisk.
...
mandatnøgle <- land %>% 
  ...
  mutate(stemmerantal = as.numeric(stemmerantal)) %>% 
  mutate(mandatnøgle = round(sum(stemmerantal)/175,2),
...
tillægsmandater <- mandatnøgle %>% 
  # TODO: bør måske afrundes
  mutate(mandatnøgle2 = if_else(para77_stk4 == TRUE, sum(stemmerantal[kreds_ant<=forholdstal])/(175-sum(kreds_ant[kreds_ant>=forholdstal])), min(mandatnøgle))) %>% 
  mutate(forholdstal2 = if_else(para77_stk4 == TRUE & kreds_ant>=forholdstal, NA_real_, stemmerantal/mandatnøgle2)) %>% 
  mutate(nedrund = floor(forholdstal2),
         rest = if_else(para77_stk4 == TRUE, 175-sum(kreds_ant[kreds_ant>=forholdstal])-sum(nedrund, na.rm = TRUE), 175-sum(nedrund)),
...
partier_kreds_ge_forholdstal <- tillægsmandater$navn[tillægsmandater$kreds_ant>=tillægsmandater$forholdstal]
...
saint_lague <- tibble(saint_lague = seq(1, 39, by = 2))

temp_tillæg <- landsdel %>% 
  inner_join(over_spær, by = "navn") %>% 
  select(landsdel, landsdel_id, navn, stemmerantal) %>% 
  cross_join(saint_lague) %>% 
