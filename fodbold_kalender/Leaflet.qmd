...
library(pacman)
p_load(tidyverse, leaflet, widgetframe, jsonlite, httr2, here, sf, quartabs)
...
## sikrer sortering
custom_order <- format(kampprogram_nested$dato, "%d. %b %Y")

kampprogram_nested <- kampprogram_nested %>% 
  mutate(dato_fac = factor(format(dato, "%d. %b %Y"), levels = custom_order))
...
render_tabset(kampprogram_nested,
              tabset_vars = dato_fac,
              output_vars = leaflet,
              pills = TRUE)
