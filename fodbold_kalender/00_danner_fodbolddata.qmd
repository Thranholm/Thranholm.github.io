...
## Finder saeson aar
liga_aar <- ifelse(month(Sys.Date()) < 7, year(Sys.Date()) - 1, year(Sys.Date()))
...
t2 <- kampprogram_fulddata %>% 
  unnest(c(league, fixture, teams, goals, score), names_sep = "_") %>% 
  unnest(c(fixture_venue, teams_home, teams_away), names_sep = "_")
```
