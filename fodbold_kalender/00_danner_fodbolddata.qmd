...
liga_get <- tibble(liga_id = c(78, 79, 80, 119),
                   get = list(NULL),
                   response = list(NULL))
...
kampprogram <- kampprogram_fulddata %>% 
  ...
  mutate(runde = str_extract(league$round, 
         ...
         kampuge = if_else(ugedag == "Mandag", isoweek(kampdato)-1, isoweek(kampdato)),
         weekend = if_else(ugedag %in% c("Fredag", "Lørdag", "Søndag", "Mandag"), TRUE, FALSE),
         ugedag_dato = str_c(ugedag, kampdato, sep = " "),
  ...
  mutate(kamptidspunkt = if_else(liga == "3. Liga" & runde != "38" & min(timestamp) == max(timestamp), 
                                 ...
                                 kamptidspunkt),
         .by = c("runde", "liga_id")) %>% 
  mutate(min_ugedag = if_else(kampdato == min(kampdato), ugedag_dato, NA), .by = fiktiv_kamprunde) %>% 
  fill(min_ugedag) %>% 
...
write_rds(kampprogram, "data/kampprogram.rds")
