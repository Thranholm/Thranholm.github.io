...
# danmark_geo_json <- request("https://api.dataforsyningen.dk/storkredse?format=geojson") %>% 
#   req_perform() %>%
#   resp_body_raw() %>%
#   rawToChar() %>%
#   st_read(quiet = TRUE) %>%
#   filter(navn != "Bornholm") %>%
#   st_make_valid() %>% 
#   st_combine()
...
kampprogram_nested <- kampprogram_geo %>% 
  ...
  mutate(plot = map(kampprogram, ~ggplot() +
                      ...
                      ggtitle(paste("Kampprogram for kamprunde med start:", unique(.x$min_ugedag))))) %>% 
    mutate(idag = today(),
         date_diff = dato-idag,
         før_idag = idag > dato) %>% 
...
girafe(ggobj = plot_grid(plotlist = kampprogram_nested$plot, ncol = 1, nrow = 25, scale = 1)
       , width_svg = 7, height_svg = 175)
