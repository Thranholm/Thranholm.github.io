...
kampprogram_danmark <- read_rds(here("fodbold_kalender", "data", "kampprogram.rds")) %>%
  ...
  mutate(stadion = case_when(spilleby == "Brondby" & is.na(stadion) ~ "Brøndby Stadion",
                             spilleby == "Aarhus" & is.na(stadion) ~ "New Vejlby Stadium",
                             .default = stadion))

if(nrow(kampprogram_danmark) == 0){
...
danske_stadion_geo <- danske_stadion %>%
  ...
  mutate(stadion_navn = case_when(primærnavnestatus == "officielt" ~ stadion,
         ...
         stadion_navn = replace_values(stadion_navn,
                                   "CASA Arena Horsens" ~ "Hybel Arena Horsens",
                                   # "Jysk Park" ~ "JYSK park",
