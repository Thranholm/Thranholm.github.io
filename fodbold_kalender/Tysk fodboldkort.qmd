...
## Stadions, hvor der har været manglende by
stadion_list_manglende_by <- tibble(stadion = c("Borussia-Park", "Merkur Spiel-Arena", "Ludwigspark Stadion",
                                                 "Sportpark Ronhof Thomas Sommer"), 
                                   spilleby = c("Monchengladbach", "Düsseldorf", "Saarbrücken", "Fürth"))

kampprogram_tyskland <- read_rds(here("fodbold_kalender", "data", "kampprogram.rds")) %>%
  filter(land == "Germany") %>%
  unnest(data) %>% 
  filter(str_detect(liga, "Regionalliga", negate = TRUE)) %>% 
  ## Manuel tilretning af bynavne, hvor der har været problemer med geodata
  mutate(spilleby = case_when(spilleby == "Wurzburger" ~ "Würzburg", 
                              .default = spilleby)) %>% 
  rows_patch(stadion_list_manglende_by, by = "stadion")
...
hjemmehold_manglende_geo <- kampprogram_geo %>% 
  filter(is.na(longitude) | is.na(latitude)) %>% 
  distinct(hjemmehold) %>% 
  pull()
...
t <- kampprogram_geo %>% 
  filter(any(is.na(spilleby)), .by = hjemmehold)


```

`r if (kampprogram_klar && !is_empty(manglende_geo)){paste0("Der er spillebyer, hvor der ikke er geo-data. Det drejer sig om: ", paste(manglende_geo, collapse = ", "), ". Det drejer sig om stadionnavnene: ", paste(manglende_stadions, collapse = ", "), ". Det drejer sig om følgende hjemmehold: ", paste(hjemmehold_manglende_geo, collapse = ", "))}`
